...
// now번 신발을 신고 갈 수 있는 최대 거리를 반환한다.
int Solve(int now)
{
    // 신거나 안 신거나 둘 중 하나의 케이스를 선택
